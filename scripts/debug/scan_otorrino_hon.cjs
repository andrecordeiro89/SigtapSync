// Varredura Otorrino (XLSX HON) em todos os hospitais (São José usa mapa específico)
...
const HOSPITAL_SAO_JOSE_ID = '792a0316-92b4-4504-8238-491d284099a3'
...
const loadXlsxMap = (filename) => {
  const candidate = [
    path.resolve(__dirname, `../../public/${filename}`),
    path.resolve(__dirname, `../../public/${encodeURIComponent(filename)}`)
  ]
...
async function scanHospital(h, mapGeneral, mapSaoJose) {
  ...
  for (const aih of (aihs || [])) {
    ...
    const map = (h.id === HOSPITAL_SAO_JOSE_ID) ? mapSaoJose : mapGeneral
    const otoProcs = procs.filter(p => {
      const code = norm(p.procedure_code || '')
      return code.startsWith('04.04.') || map.has(code)
    })
    if (otoProcs.length === 0) continue
    checked++
    let pos = 0
    const paidCodes = new Set()
    let total = 0
    for (const p of otoProcs) {
      const code = norm(p.procedure_code || '')
    ...
    if (total === 0) {
      zeroPayments++
      const codes = otoProcs.map(p => ({ code: norm(p.procedure_code || ''), cbo: p.professional_cbo || '' }))
...
async function run() {
  const mapOto = loadXlsxMap('VBA OTORRINO.xlsx')
  const mapOtoSj = loadXlsxMap('VBA OTORRINO HOSPITAL MUNICIPAL SÃO JOSE.xlsx')
  console.log(`✅ Oto geral: ${mapOto.size} códigos • Oto São José: ${mapOtoSj.size} códigos`)
  ...
  for (const h of (hospitals || [])) {
    const r = await scanHospital(h, mapOto, mapOtoSj)
    results.push(r)
  }
  console.log('RESUMO GERAL (Otorrino)')
  results.forEach(r => {
    console.log(`${r.name}: AIHs com OTO ${r.checked} | repasse zero ${r.zeroPayments}`)
    if (r.zeroPayments > 0) {
