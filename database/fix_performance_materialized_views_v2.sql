-- ================================================================
-- SOLUÇÃO DE PERFORMANCE: MATERIALIZED VIEWS (VERSÃO CORRIGIDA)
-- ================================================================
...
-- ================================================================
-- PASSO 1: CRIAR ÍNDICES ESSENCIAIS (SE NÃO EXISTIREM)
-- ================================================================
DO $$ 
BEGIN
...
-- ================================================================
-- PASSO 2: REMOVER VIEWS ANTIGAS (SE EXISTIREM)
-- ================================================================
...
-- Remover views materializadas antigas (se existirem)
DROP MATERIALIZED VIEW IF EXISTS v_hospital_revenue_stats CASCADE;
DROP MATERIALIZED VIEW IF EXISTS v_specialty_revenue_stats CASCADE;
DROP MATERIALIZED VIEW IF EXISTS v_doctors_aggregated CASCADE;
DROP MATERIALIZED VIEW IF EXISTS v_doctor_revenue_monthly CASCADE;

-- ================================================================
-- PASSO 3: CRIAR MATERIALIZED VIEWS
-- ================================================================
...
SELECT 
  ...
  EXTRACT(YEAR FROM pr.procedure_date)::integer as revenue_year,
  EXTRACT(MONTH FROM pr.procedure_date)::integer as revenue_month,
  ...
  -- Valores financeiros
  COALESCE(SUM(pr.value_charged), 0) as total_revenue_cents,
  ROUND(COALESCE(SUM(pr.value_charged), 0) / 100.0, 2) as total_revenue_reais,
...
SELECT 
  ...
  -- Hospitais agrupados (SEM DISTINCT para permitir ORDER BY)
  STRING_AGG(h.name, ' | ' ORDER BY h.name) FILTER (WHERE h.name IS NOT NULL) as hospitals_list,
  STRING_AGG(h.id::text, ',' ORDER BY h.id) FILTER (WHERE h.id IS NOT NULL) as hospital_ids,
  COUNT(DISTINCT h.id) FILTER (WHERE h.id IS NOT NULL) as hospitals_count,
  ...
  ROUND(COALESCE(AVG(drm.payment_rate_percent), 0), 2) as avg_payment_rate_12months,
...
SELECT 
  ...
  COALESCE(SUM(total_revenue_12months_cents), 0) as total_specialty_revenue_cents,
  ROUND(COALESCE(SUM(total_revenue_12months_cents), 0) / 100.0, 2) as total_specialty_revenue_reais,
  ROUND(COALESCE(AVG(total_revenue_12months_cents), 0) / 100.0, 2) as avg_doctor_revenue_reais,
  COALESCE(SUM(total_procedures_12months), 0) as total_procedures,
  ROUND(COALESCE(AVG(total_procedures_12months), 0), 0) as avg_procedures_per_doctor,
  ROUND(COALESCE(AVG(avg_payment_rate_12months), 0), 2) as avg_payment_rate
...
SELECT 
  ...
  COUNT(DISTINCT dh.doctor_id) FILTER (WHERE dh.doctor_id IS NOT NULL) as active_doctors_count,
  COUNT(DISTINCT dh.doctor_id) FILTER (WHERE da.activity_status = 'ATIVO') as very_active_doctors,
  ...
  ROUND(COALESCE(AVG(da.total_revenue_12months_cents), 0) / 100.0, 2) as avg_doctor_revenue_reais,
  
  COALESCE(SUM(da.total_procedures_12months), 0) as total_procedures,
  ROUND(COALESCE(AVG(da.total_procedures_12months), 0), 0) as avg_procedures_per_doctor,
  ROUND(COALESCE(AVG(da.avg_payment_rate_12months), 0), 2) as avg_payment_rate
...
-- ================================================================
-- PASSO 4: FUNÇÃO PARA REFRESH AUTOMÁTICO
-- ================================================================
...
EXCEPTION
  WHEN OTHERS THEN
    -- Se CONCURRENTLY falhar (primeira vez), tentar sem CONCURRENTLY
    ...
    RAISE NOTICE 'Views materializadas atualizadas (sem CONCURRENTLY) às %', NOW();
END;
$$ LANGUAGE plpgsql;

-- ================================================================
-- PASSO 5: POPULAR AS VIEWS INICIALMENTE
-- ================================================================
DO $$
BEGIN
  RAISE NOTICE 'Populando views materializadas... Isso pode levar alguns minutos.';
  
  REFRESH MATERIALIZED VIEW v_doctor_revenue_monthly;
  RAISE NOTICE 'View 1/4 populada: v_doctor_revenue_monthly';
  
  REFRESH MATERIALIZED VIEW v_doctors_aggregated;
  RAISE NOTICE 'View 2/4 populada: v_doctors_aggregated';
  
  REFRESH MATERIALIZED VIEW v_specialty_revenue_stats;
  RAISE NOTICE 'View 3/4 populada: v_specialty_revenue_stats';
  
  REFRESH MATERIALIZED VIEW v_hospital_revenue_stats;
  RAISE NOTICE 'View 4/4 populada: v_hospital_revenue_stats';
  
  RAISE NOTICE 'Todas as views foram populadas com sucesso!';
END $$;
...
-- COMENTÁRIOS
-- ================================================================
COMMENT ON MATERIALIZED VIEW v_doctor_revenue_monthly IS 
'Faturamento mensal por médico (últimos 13 meses) - Base para cálculos agregados';

COMMENT ON MATERIALIZED VIEW v_doctors_aggregated IS 
'Médicos agregados sem duplicação com múltiplos hospitais e faturamento 12 meses';

COMMENT ON MATERIALIZED VIEW v_specialty_revenue_stats IS 
'Estatísticas de faturamento agrupadas por especialidade médica';

COMMENT ON MATERIALIZED VIEW v_hospital_revenue_stats IS 
'Estatísticas de faturamento agrupadas por hospital';

COMMENT ON FUNCTION refresh_revenue_views() IS 
'Atualiza todas as views materializadas de faturamento';

-- ================================================================
-- INSTRUÇÕES DE USO:
-- 1. Execute este script UMA VEZ no SQL Editor do Supabase
-- 2. Aguarde a mensagem "Todas as views foram populadas com sucesso!"
-- 3. Verifique o resultado final da query SELECT no final
-- 4. Para atualizar as views manualmente: SELECT refresh_revenue_views();
-- ================================================================
