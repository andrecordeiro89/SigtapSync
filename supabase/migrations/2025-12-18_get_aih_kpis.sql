...
as $$
  select
    s.total_aihs,
    s.total_value_cents / 100.0 as total_value,
    case when s.total_aihs > 0 then (s.total_value_cents / 100.0) / s.total_aihs else 0 end as average_ticket,
    s.approved_aihs,
    case when s.total_aihs > 0 then (s.approved_aihs::numeric * 100.0) / s.total_aihs else 0 end as approval_rate
  from (
    select
      count(distinct a.id) as total_aihs,
      coalesce(sum(a.calculated_total_value), 0) as total_value_cents,
      sum(case when a.processing_status = 'approved' then 1 else 0 end) as approved_aihs
    from public.aihs a
...
grant execute on function public.get_aih_kpis to authenticated;
