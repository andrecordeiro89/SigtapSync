-- Partial unique indexes to avoid existing bad data collisions
create unique index if not exists ux_patients_hospital_cns_partial on public.patients(hospital_id, cns) where cns is not null and cns <> '';
create unique index if not exists ux_aihs_hospital_number_partial on public.aihs(hospital_id, aih_number) where aih_number is not null and aih_number <> '' and aih_number <> '-';

-- Ensure function exists (re-create to be safe)
...
begin
  ...
  on conflict on constraint ux_patients_hospital_cns_partial do update set name = excluded.name, birth_date = excluded.birth_date, gender = excluded.gender, address = excluded.address, city = excluded.city, state = excluded.state, zip_code = excluded.zip_code, phone = excluded.phone, email = excluded.email, medical_record = excluded.medical_record;
