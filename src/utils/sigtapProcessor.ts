...
export const processSigtapZip = async (file: File): Promise<SigtapProcessingResult> => {
  try {
    ...
    const result: SigtapProcessingResult = {
      success: true,
      message: `Arquivo ${file.name} processado com sucesso! ${mockProcedures.length} procedimentos importados.`,
...
const generateMockProceduresFromFile = (filename: string): SigtapProcedure[] => {
  ...
  for (let i = 0; i < 50; i++) {
    additionalProcedures.push({
      ...
      description: `PROCEDIMENTO SIMULADO ${i + 1} - ${filename.replace('.zip', '').toUpperCase()}`,
