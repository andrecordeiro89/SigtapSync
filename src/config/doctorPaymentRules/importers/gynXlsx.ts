...
export const getGynHonValuesForCode = async (code: string): Promise<HonValues | null> => {
  const m = await loadGynHonMap()
  const normalized = code.match(/^(\d{2}\.\d{2}\.\d{2}\.[\d]{3}-\d)/)?.[1] || code
  return m.get(normalized) || null
}

export const getGynHonMapSync = (): Map<string, HonValues> | null => {
  return GYN_HON_MAP
}
...
export const calculateGynHonPayments = async (procedures: ProcedurePaymentInfo[]): Promise<CalculatedPaymentResult> => {
  const map = await loadGynHonMap()
  let total = 0
  let pos = 0
  const paidCodes = new Set<string>()
  const out = await Promise.all(procedures.map(async (p) => {
    ...
    return {
      ...
      paymentRule: isDuplicate ? 'Duplicado (não pago)' : (hon ? `GINECOLOGIA HON (pos ${idx+1})` : 'Sem regra HON para código'),
      isSpecialRule: true
    }
  }))
  return { procedures: out, totalPayment: total, appliedRule: 'XLSX Ginecologia HON1..HON5' }
}

export const calculateGynHonPaymentsSync = (procedures: ProcedurePaymentInfo[]): CalculatedPaymentResult | null => {
  const map = getGynHonMapSync()
  if (!map) return null
  let total = 0
  let pos = 0
  const paidCodes = new Set<string>()
