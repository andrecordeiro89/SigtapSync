...
let ORT_HON_MAP: Map<string, HonValues> | null = null
let initPromise: Promise<Map<string, HonValues>> | null = null

const candidateUrls: string[] = [
  '/VBA%20ORTOPEDIA.xlsx',
  '/VBA ORTOPEDIA.xlsx'
]
let resolvedUrl: string | undefined = candidateUrls[0]

export const loadOrtHonMap = async (): Promise<Map<string, HonValues>> => {
  if (ORT_HON_MAP) return ORT_HON_MAP
  ...
  initPromise = (async () => {
    try {
      ...
      for (const row of rows.slice(1)) {
        ...
        ORT_HON_MAP?.set(code, { hon1, hon2, hon3, hon4, hon5 })
...
export const getOrtHonMapSync = (): Map<string, HonValues> | null => {
  return ORT_HON_MAP
}
...
export const calculateOrtHonPaymentsSync = (procedures: ProcedurePaymentInfo[]): CalculatedPaymentResult | null => {
  const map = getOrtHonMapSync()
  if (!map) return null
  let total = 0
  let pos = 0
  ...
  const out = procedures.map((p) => {
    ...
    return {
      ...
      paymentRule: isDuplicate ? 'Duplicado (não pago)' : (hon ? `ORTOPEDIA HON (pos ${idx+1})` : 'Sem regra HON para código'),
      isSpecialRule: true
    }
  })
  return { procedures: out, totalPayment: total, appliedRule: 'XLSX Ortopedia HON1..HON5' }
}
