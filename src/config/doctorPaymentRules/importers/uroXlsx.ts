...
let URO_HON_MAP: Map<string, HonValues> | null = null
let initPromise: Promise<Map<string, HonValues>> | null = null

const assetUrl: string | undefined = (() => {
  try {
    // @ts-ignore
    const url = new URL('@/assets/VBA_UROLOGIA.xlsx', import.meta.url)
    return url as unknown as string
  } catch {
    try {
      return '/VBA%20UROLOGIA.xlsx'
    } catch {
      return undefined
    }
  }
...
export const loadUroHonMap = async (): Promise<Map<string, HonValues>> => {
  if (URO_HON_MAP) return URO_HON_MAP
...
export const getUroHonMapSync = (): Map<string, HonValues> | null => {
  return URO_HON_MAP
}
...
export const calculateUroHonPaymentsSync = (procedures: ProcedurePaymentInfo[]): CalculatedPaymentResult | null => {
  const map = getUroHonMapSync()
  if (!map) return null
  let total = 0
  let pos = 0
  ...
  return { procedures: out, totalPayment: total, appliedRule: 'XLSX Urologia HON1..HON5' }
}
